# Vagas remotas de UI / UX / Visual Design em games, aceitando candidatos em São Paulo

**Data da pesquisa:** 24/09/2026. **Perfil:** Senior Visual Designer, hoje na Kazoo Games e antes na Wildlife Studios e em agências (Tribal, SunsetDDB, BSUR). Pontos fortes: marca, UI, ilustração e arte de marketing.

## Como a lista foi feita

- **O acesso direto às páginas de vaga (Greenhouse, Lever, LinkedIn, Himalayas…) está bloqueado neste ambiente.** Os dados vêm de cerca de 180 buscas na web e dos trechos de resultado.
- **Nenhuma vaga foi aberta nem confirmada.** Status, data e elegibilidade são indicativos, então abra cada link antes de se candidatar.
- **Todas as URLs apareceram nos resultados de busca.** Nenhuma foi montada à mão.
- **A cota de buscas da sessão acabou antes do fim.** Veja a lista de estúdios não pesquisados no final.
- **Confiança SP:** chance de a vaga aceitar alguém que mora em São Paulo.
- **Fit:** quanto a vaga combina com o seu perfil.

---

## 1. Prioridade máxima: aceita Brasil e tem fit alto

| # | Empresa | Vaga | Plataforma | Remoto / local | Evidência de que aceita Brasil | Data | Confiança SP | Fit | Link |
|---|---|---|---|---|---|---|---|---|---|
| 1 | **Playrix** (Gardenscapes, Homescapes, top-grossing mobile) | Principal UI Artist | Mobile | "remote (available anywhere)" | Time remoto em mais de 50 países | set/2026 | Alta | **Alto** | https://playrix.com/job/open/art/principal-ui-artist · https://hitmarker.net/jobs/playrix-principal-ui-artist-1520911 |
| 2 | **Playrix** | Lead UI Artist | Mobile | Remoto global | Mesmo modelo remoto global | 2026 | Média-Alta | **Alto** | https://playrix.com/job/open/art/lead-ui-artist |
| 3 | **Playrix** | Senior UI Artist | Mobile | Remoto global | Mesmo modelo remoto global | 2026 | Média-Alta | **Alto** | https://playrix.com/job/open/art/senior-ui-artist |
| 4 | **Fortis Games** (financiada pela Tencent, Rumble Club) | Lead Graphic Designer / Illustrator | Mobile | "Remote (Work from Home)" | Já tem funcionários no Brasil e contrata em "US, Canada, Brazil, Spain, UK" | 2026 | Alta | **Alto**: marca, ilustração e marketing em nível Lead | https://himalayas.app/companies/fortis-games/jobs/lead-graphic-designer-ilustrator-1079262713 · https://startup.jobs/lead-graphic-designer-ilustrator-fortis-games-7991325 |
| 5 | **Fortis Games** | Staff 2D UI Artist | Mobile/PC | Remoto | A empresa contrata no Brasil; esta vaga não cita o país | 2026 | Média | **Alto** | https://gamejobs.co/Staff-2D-UI-Artist-at-Fortis-Games · https://himalayas.app/companies/fortis-games/jobs/staff-2d-ui-artist-5290709535 |
| 6 | **Gamebreaking Studios** (co-dev de UI embutido num estúdio AAA; já trabalhou para Riot, Blizzard e Microsoft) | UI Artist (Contract, pode virar efetivo) | PC/console (Unreal, Figma) | "open to candidates living and working in Latin America" | Diz "Latin America" e foi listada como "São Paulo, 4 days ago" | set/2026 | Alta | **Alto**: UI para cliente AAA | https://gamebreaking-studios-inc.breezy.hr/p/fea6283be99f-ui-artist-contract · https://gamejobs.co/UI-Artist-Contract-at-Gamebreaking-Studios |
| 7 | **Gamebreaking Studios** | Lead / Senior UI Artist (contract-to-hire) | PC/console | "Remote from Brazil, Colombia, Peru, Mexico" | O título da vaga cita o Brasil | Contrato ia até 21/08/2026, **pode ter fechado** | Alta | **Alto** | https://ixdf.org/jobs/canada-latam-lead-senior-ui-artist-gamebreaking-studios-remote-from-brazil-colombia-peru-mexico |
| 8 | **Kokku** (Recife; co-dev de Horizon, CoD e Warner) | UI/UX, banco de talentos | Console/PC/mobile | Remoto (o banco de talentos da empresa é "permanent, full time, remote") | Empresa brasileira | Sempre aberto | Alta | **Alto**: caminho mais direto para AAA a partir do Brasil | https://kokkugames.factorialhr.com.br/job_posting/ui-ux-talent-pool-239785 |
| 9 | **Kokku** | Creative Manager | — | Brasil | Empresa brasileira | ? | Alta | **Alto**: marca e marketing | https://kokkugames.factorialhr.com.br/job_posting/creative-manager-311462 |
| 10 | **Room 8 Studio** (UI de Apex Legends, Deathloop e Dead Space; tem escritório no Brasil) | UI/UX Designer (teste aberto) | Mobile/PC | Candidatura aberta; tem time no Brasil | Brasil está entre os locais do estúdio | Sempre aberto | Média-Alta | **Alto** | https://room8studio.com/open-test/ui-ux-designer/ |
| 11 | **Globant Games** (atende AAA e mobile) | Senior Game UX/UI Designer (Anywhere) | PC/console/mobile | "Anywhere" (vale para a América Latina) | A Globant usa "Anywhere" para contratar em qualquer país da LATAM | ? | Média | **Alto** | https://career.globant.com/job/Caba-Game-UXUI-Designer-(Anywhere)-Arge/556922317 |
| 12 | **Globant Games** | Game UX/UI Designer | PC/console/mobile | Brasil ("Other City") | Vaga listada no Brasil | ? | Média-Alta (pode ser híbrida) | **Alto** | https://career.globant.com/job/Other-City-Game-UXUI-Designer/558476217 |
| 13 | **Team Liquid** (esports) | Senior Graphic Designer, BR (PJ) | Esports/marca | Brasil, PJ | O título diz "BR" | ? | Alta | **Alto**: pede Blender ou C4D | https://careers.teamliquid.com/jobs/5926002-senior-graphic-designer |
| 14 | **YallaPlay** | Senior Mobile Game UI/UX Artist | Mobile | Remoto | Tem operação no Brasil | 2026 | Média-Alta | **Alto** | https://yallaplay.breezy.hr/p/e5f3ebfd5910-senior-mobile-game-ui-ux-artist |
| 15 | **Gazeus Games** | Senior UX/UI Artist, Mobile Games | Mobile | Remoto no Brasil (em anúncios anteriores) | Empresa brasileira | ? | Média | **Alto** | https://gamejobs.co/Senior-UX-UI-Artist-Mobile-Games-at-Gazeus-Games |
| 16 | **Amber** (tem estúdio em SP) | UI Artist (por projeto) | Mobile | "Trabalho remoto" | Listada no Glassdoor Brasil | ? | Média | **Alto** | https://jobs.jobvite.com/amberstudiocareers/job/omdgufw3 · https://hitmarker.net/jobs/amber-studio-ui-artist-project-based-1466261 |
| 17 | **Moon Active** (Coin Master) | UI Artist (contractor) | Mobile | "worldwide remote" | Diz "worldwide" | mar/2026, **pode ter fechado** | Média | **Alto**: contrato PJ | https://www.remoterocketship.com/us/company/moonactive/jobs/ui-artist-worldwide-remote/ |

## 2. Também vale olhar: fit médio ou dúvida sobre elegibilidade

| Empresa | Vaga | Remoto / local | Confiança SP | Fit | Link |
|---|---|---|---|---|---|
| **Epic Games** (Fortnite, **AAA**) | Tech UI Designer / Lead Tech UI Designer | Porto Alegre e São Paulo; um anúncio diz "open for remote hiring within Brazil" | Média | Médio: é implementação em Unreal (UMG/Slate), não visual | https://gamejobs.co/Tech-UI-Designer-at-Epic-Games-4705 · https://gamejobs.co/Lead-Tech-UI-Designer-at-Epic-Games-1469 |
| **Epic Games** | Artista de UI (R27295) | Porto Alegre, **presencial** | Baixa (exige mudança) | Alto nas habilidades, mas é nível pleno | https://www.epicgames.com/site/careers/jobs/6000100004 |
| **Epic Games** | UX Designer | Brasil (a confirmar) | Baixa | Médio | https://gamejobs.co/UX-Designer-at-Epic-Games-4871 |
| **Riot Games** (**AAA**) | Marketing Creative Director II/III | Escritório de São Paulo | Alta, se ainda estiver aberta (o ID é antigo) | Alto: marketing, marca e agência | https://www.riotgames.com/en/work-with-us/job/3674007/marketing-creative-director-iii-sao-paulo-brazil |
| **Moon Active** | AI Designer (UI e arte com IA generativa) | "remote global position" | Média-Alta | Médio/Alto: exige ComfyUI e ferramentas de IA generativa | https://gamejobs.co/AI-Designer-at-Moon-Active |
| **Fortis Games** | Motion Designer (contractor) / Lead Motion Designer | "Brazil Remote" | Alta | Médio | https://www.remoterocketship.com/company/fortisgames/jobs/motion-designer-contractor-brazil-remote/ |
| **Airship Interactive** (arte externa para AAA) | Freelance UI Artist / Freelance UI/UX Designer | "fully remote", pool global de freelancers | Média | Alto (freelance) | https://careers.airshipinteractive.com/jobs/8216442-freelance-ui-artist · https://careers.airshipinteractive.com/jobs/8009772-freelance-ui-ux-designer |
| **Gamebreaking Studios** | Lead / Senior UX Designer | Canadá ou América Latina | Alta | Médio: pede UX puro | https://gamejobs.co/Canada-Latin-America-Lead-Senior-UX-Designer-Contract-to-hire-at-Gamebreaking-Studios |
| **Unknown Worlds** (Krafton, Subnautica 2) | UI Artist / Technical UI Designer | "globally distributed team"; um anúncio diz que o local é EUA | Baixa-Média | Alto na disciplina | https://job-boards.greenhouse.io/unknownworlds/jobs/8169957002 · https://job-boards.greenhouse.io/unknownworlds/jobs/8625252002 |
| **iLogos** (outsourcing para Sony e EA) | UI/UX Designer, GameDev | "Worldwide Remote" | Média | Médio | https://www.remoterocketship.com/company/careers-ilogos-biz/jobs/ui-ux-designer-gamedev-worldwide-remote/ |
| **UserWise** | Senior UI Motion & VFX Artist (F2P mobile) | "100% remote", "prioritizing candidates based in Brazil" | **Alta** | Médio: foco em motion e VFX | https://apply.workable.com/userwise-services/j/6D605DC6A9 |
| **Homa Games** | Motion Designer / Marketing Video Artist (freelance, criativos de UA) | Remoto worldwide | Média-Alta | Médio | https://apply.workable.com/homa-games/j/409308F04C/ |
| **Side (ex-PTW)** | UI/UX Artist (simulador em VR, UE5) | Estado de São Paulo | Média | Médio: pede experiência em VR | https://freehire.me/jobs/ui-ux-artist-ptw-q73z6w25 |
| **Globant Games** | Gaming Tech UI Designer (Senior) | São Paulo (provavelmente híbrido) | Média | Baixo-Médio | https://www.hubmub.com/jobs/869564/gaming-tech-ui-designer |
| **Miniclip** | Senior UI Artist | Local não informado | Baixa | Alto, se aceitar Brasil | https://www.miniclip.com/careers/vacancies/senior-ui-artist/1651 |
| **Wildlife Studios** (ex-empregadora) | Visual Designer / UI/UX Game Artist / Lead Designer | São Paulo, híbrido | Alta | Baixo: seria um movimento lateral | https://job-boards.greenhouse.io/wildlifestudios/jobs/8635029002 |
| Indies (TigerRoll, Memory Ware, Razorhead, Half Mermaid, Double Twizzle) | UI Artist | Remoto worldwide ou contrato | Média | Médio | https://www.workwithindies.com/?categories=art-animation |

## 3. Vagas AAA de UI/visual que não aceitam remoto do Brasil

Servem como referência ou para candidatura com relocação.

| Empresa | Vaga | Restrição | Link |
|---|---|---|---|
| EA | Lead/Senior Game Visual UI Designer | Kuala Lumpur | https://gamejobs.co/Lead-Senior-Game-Visual-UI-Designer-at-Electronic-Arts |
| EA SPORTS FC | Experience Designer (Visual Craft & UI) | Vancouver/Orlando, híbrido | https://gamejobs.co/Experience-Designer-Visual-Craft-and-UI-EA-SPORTSTM-FC-at-Electronic-Arts |
| CD Projekt Red | Senior UI Designer | Remoto só em alguns estados dos EUA, ou Varsóvia | https://builtin.com/job/senior-ui-designer/9195322 |
| King | Senior UI Artist (New Games) | Londres, Barcelona ou Estocolmo | https://gamejobs.co/Senior-UI-Artist-New-Games-at-King-9372 |
| Scopely | Senior UI Artist (Monopoly GO!) | Barcelona ou Dublin | https://gamejobs.co/Senior-UI-Artist-Monopoly-GO-at-Scopely-8802 |
| Ubisoft | UI/UX Designer, Mobile | Barcelona | https://www.ubisoft.com/en-us/company/careers/search/744000077247826-ui-ux-designer-mobile-gaming- |
| Paradox | 2D/UI Artist (Crusader Kings 3) | Estocolmo | https://career.paradoxplaza.com/jobs/8350481-2d-ui-artist |
| Blizzard | UI Artist (jogo de sobrevivência não anunciado) | EUA | https://gamejobs.co/UI-Artist-Unannounced-Survival-Game-at-Blizzard-Entertainment |
| Riot | Visual Design Artist, LoL (contrato) | Los Angeles | https://www.riotgames.com/en/work-with-us/job/7997410?gh_jid=7997410 |
| NetEase | Lead UI/UX Designer | Remoto só na União Europeia | https://uiuxjobsboard.com/job/765623-lead-ui-ux-designer |
| Virtuos | Lead / Senior UI/UX Designer | França, Praga ou remoto na Europa | https://remotive.com/remote/jobs/design/lead-ui-ux-designer-3419131 |
| Sony PlayStation | Senior UI/UX Designer | Provavelmente EUA | https://job-boards.greenhouse.io/sonyinteractiveentertainmentglobal/jobs/6011571004 |

**Padrões por empresa**
- **Contratam remoto só nos EUA ou na Europa:** Bungie, Activision Blizzard, Scopely, Jam City, Zynga, Playtika, Supercell ("none of our openings are remote") e Sumo Digital.
- **No Brasil:**
  - **Epic** contrata, mas em Porto Alegre.
  - **Riot** tem em São Paulo publishing e marketing. As vagas de arte de jogo ficam em LA.

## 4. Onde acompanhar

- gamejobs.co, busca "Remote - Brazil": https://gamejobs.co/search?w=Remote+-+Brazil
- Playrix, vagas de arte: https://playrix.com/job/open/art
- Remote Rocketship, design gráfico na LATAM: https://www.remoterocketship.com/country/latin-america/jobs/graphics-design/
- Himalayas, UX/UI no Brasil: https://himalayas.app/jobs/countries/brazil/ux-ui-design
- Hitmarker, vagas remotas (e o perfil @HitmarkerBR no X): https://hitmarker.net/remote-gaming-jobs
- uiuxjobsboard, remoto no Brasil: https://uiuxjobsboard.com/design-jobs/remote-brazil
- Jobgether, UX/UI na LATAM: https://jobgether.com/remote-jobs/latam/ux-ui-designer
- Páginas de carreira de Epic (filtro Brasil), Kokku, Room 8, Globant, Gamebreaking, Fortis e Amber

**Palavras-chave para alertas:** "UI Artist" LATAM, "Brazil Remote", "Latin America" + "UI", "Graphic Designer" games Brazil, PJ + games + designer.

## 5. Ficaram de fora por causa do limite de buscas

Supercell (só a política remota foi checada), King (só vagas presenciais), Netmarble, Com2uS, Niantic, Pixel United, Mediatonic, Moonton, Dream Games, Rollic, Big Fish, Product Madness, Hoyoverse, Level Infinite, Wargaming, Rockstar, Valve, Bethesda, 2K, Square Enix, Bandai Namco, Sega, Capcom, Behaviour, Saber, Sperasoft.
